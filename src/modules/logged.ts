...
const displayCurrentUser = () => {
  const currentUserElement: HTMLSpanElement | null =
    document.getElementById("current-user");
  const currentUserImgContainer = document.getElementById(
    "current-user-img-container"
  ) as HTMLDivElement;
  const currentUserImg = document.createElement("img") as HTMLImageElement;
...
async function displayUsers() {
  try {
    ...
    const usersArray = (await getUsers()) as User[];
    ...
    usersArrayWithoutCurrentUser.forEach((user) => {
      ...
      const otherUsersContainer = document.getElementById("other-users") as HTMLUListElement;
